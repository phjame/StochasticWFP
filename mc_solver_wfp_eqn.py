...
def main():
	# Parse command-line arguments
        ...
        parser.add_argument("-N", nargs="?", type=float, default=1e3, help="Number of points to sample")
        parser.add_argument("-T", nargs="?", type=float, default=20, help="Total time")
        parser.add_argument("-dT", nargs="?", type=float, default=0.1, help="Time step")
        parser.add_argument("--store_final", action=argparse.BooleanOptionalAction, help="Whether or not to store the final plot points")
        parser.add_argument("--store_all", action=argparse.BooleanOptionalAction, help="Whether or not to store the all plot points")
        ...
        if args.store_final:
                np.save(f"Numerical_Solution_Final_N_{N}_T_{t_tot}_dT_{dt}", u)

        # Optionally save results of all plot points into NumPy array
        if args.store_all:
                np.save(f"Numerical_Solution_All_N_{N}_T_{t_tot}_dT_{dt}", u_all)
